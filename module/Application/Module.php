    ...
    public function onBootstrap(MvcEvent $e)
    {
      ...
      /*  
        ...
        $sharedEventManager->attach('Zend\Mvc\Controller\AbstractActionController', 'record.post', function ($e) {
       	...
       	$sharedEventManager->attach('Zend\Mvc\Controller\AbstractController', 'sensor.deleted', function ($e) {
       		$sm = $e->getTarget()->getServiceLocator()->get('search-manager');
       		try {
       			$sm->delete('sensors', $e->getParams()['sensor']);
       		} catch (Exception $e) {
       			//do something if es is down for example (mark record as to be indexed later for example
       		}
       	});
    		...
    		'factories' =>
    			array (
    				...
    				'sensor' => function($sm) {
    					...
    					$dbAdapter = $sm->getServiceLocator()->get('Zend\Db\Adapter\Adapter');
    					$form->setInputFilter(new \Application\InputFilters\Sensor($dbAdapter));
    					$form->setHydrator(new ClassMethods());
    					return $form;
    				},
    		)
